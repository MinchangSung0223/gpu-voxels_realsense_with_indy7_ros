...
int main(int argc, char* argv[])
{
  ...
  const Vector3f camera_offsets(1.5f+0.89f, 1.5f-0.04f, 1.08f); // camera located at y=-0.2m, x_max/2, z_max/2
  ...
  ros::Subscriber sub1 = nh.subscribe("joint_states", 1, jointStateCallback); 
  ...
  while (ros::ok())
  {
    ...
    if (new_data_received) 
    {
      ...
      num_colls = gvl->getMap("countingVoxelList")->as<gpu_voxels::voxellist::CountingVoxelList>()->collideWith(gvl->getMap("myRobotMap")->as<gpu_voxels::voxellist::BitVectorVoxelList>(), 1.0f);
